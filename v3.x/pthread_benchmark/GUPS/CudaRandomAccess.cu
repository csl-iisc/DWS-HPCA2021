...
/* #define NUPDATE (1024*1024) */
#define NUPDATE (1024 * 120)
#define THREAD_BLOCK 15
#define NTHREAD_BLOCKS (32)
#define MAX_VL 128
...
#define NF_ATOMIC
#ifdef NF_ATOMIC
extern "C" __device__ __noinline__ void _Z_intrinsic_atom_global_xor_nf_i64(
    void* address,
...
#else
extern "C" __device__ __noinline__ unsigned long long int
_Z_intrinsic_atom_global_xor_i64(void* address, unsigned long long int data) {
...
// CUDA kernel
__global__ static void RandomAccessUpdate(u64Int TableSize,
                                          u64Int* Table,
                                          u64Int* starts) {
  u64Int i;
   ...
   *     }
   */
  // ran = HPCC_starts ((NUPDATE/THREAD_BLOCK/NTHREAD_BLOCKS) * (blockIdx.x *
...
int HPCC_RandomAccess(Params_t* params,
                      int doIO,
                      double* GUPs,
                      int* failure,
  ...
  if (!Table) {
    if (doIO) {
      fprintf(outFile,
              "Failed to allocate memory for the update table (" FSTR64 ").\n",
              TableSize);
      fclose(outFile);
    }
    return 1;
  ...
  for (i = 0; i < (THREAD_BLOCK * NTHREAD_BLOCKS); i++)
    starts[i] = HPCC_starts((NUPDATE / THREAD_BLOCK / NTHREAD_BLOCKS) * i);
#endif
...
/* Begin timing here */
  ...
  cutilSafeCall(cudaMemcpyAsync(d_Table, Table, size, cudaMemcpyHostToDevice,
                                stream_app));
  cutilSafeCall(cudaMemcpyAsync(d_starts, starts, sizeof(starts),
                                cudaMemcpyHostToDevice, stream_app));

    cutilSafeCall(cudaStreamSynchronize(stream_app));
/*   cutilSafeCall(cudaMemcpy(d_Table, Table, size, cudaMemcpyHostToDevice)); */
/*   cutilSafeCall(cudaMemcpy(d_starts, starts, sizeof(starts), */
/*                                 cudaMemcpyHostToDevice)); */

  RandomAccessUpdate<<<NTHREAD_BLOCKS, THREAD_BLOCK, 0, stream_app>>>(
      TableSize, d_Table, d_starts);

  cutilSafeCall(cudaGetLastError());

  cutilSafeCall(cudaMemcpyAsync(Table, d_Table, size, cudaMemcpyDeviceToHost,
                                stream_app));

/*   cutilSafeCall(cudaMemcpy(Table, d_Table, size, cudaMemcpyDeviceToHost)); */
