...
////////////////////////////////////////////////////////////////////////////////
extern "C" void convolutionRowCPU(float* h_Result,
                                  float* h_Data,
                                  float* h_Kernel,
                                  int imageW,
                                  int imageH,
...
extern "C" void convolutionColumnCPU(float* h_Result,
                                     float* h_Data,
                                     float* h_Kernel,
                                     int imageW,
                                     int imageH,
...
int main_CONS(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  cutilSafeCall(cudaMemcpyAsync(d_Input, h_Input,
  ...
  for (unsigned int i = 0; i < iterations; i++) {
    convolutionRowsGPU(d_Buffer, d_Input, imageW, imageH, stream_app);

    convolutionColumnsGPU(d_Output, d_Buffer, imageW, imageH, stream_app);
  }
  // cutilSafeCall( cudaThreadSynchronize() );
  ...
  printf("Average GPU convolution time : %f msec //%f Mpixels/sec\n", gpuTime,
         1e-6 * imageW * imageH / (gpuTime * 0.001));

  printf("Reading back GPU results...\n");
  cutilSafeCall(cudaMemcpyAsync(h_OutputGPU, d_Output,
                                imageW * imageH * sizeof(float),
