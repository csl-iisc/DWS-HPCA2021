...
/*void addBenchmarkSpecOptions(OptionParser &op){
    ...
    op.addOption("TextureMem", OPT_BOOL, "0", "Use Texture memory for distance
matrix");
    op.addOption("CompactStorage", OPT_BOOL, "0", "Use compact storage distance
matrix regardless of problem size");
...
//
// Purpose:
//   This function decides how many GPUs (up to the maximum requested by the
//   user)
//   and threadblocks per GPU will be used. It also returns the total number of
//   thread-blocks across all GPUs and the number of thread-blocks that are in
//   nodes
...
//
// ****************************************************************************
void calculate_participants(int point_count,
                            int node_count,
                            int cwrank,
...
unsigned long int estimate_memory_for_full_storage(unsigned long int pnt_cnt,
                                                   float d) {
  ...
  // The density calculations assume that we are dealing with generated
  // Euclidean points
  // (as opposed to externally provided scientific data) that are constraint in
  // a 20x20 2D square.
  density = 3.14159 * (d * d) / (20.0 * 20.0);
  if (density > 1.0)
    density = 1.0;
  max_degree = (unsigned long int)((float)pnt_cnt * density);  // average number
                                                               // of points in a
                                                               // cirlce with
                                                               // radius d.
  max_degree *= 10;  // The distribution of points is not uniform, so throw in a
                     // factor of 10 for max/average.
  if (max_degree > pnt_cnt)
    max_degree = pnt_cnt;
  // Due to the point generation algorithm, a cluster can have up to N/30
  // elements in an arbitratiry small radius.
  ...
  total += pnt_cnt * thread_block_count *
           sizeof(char);           // Current candidate cluster mask
...
void runTest(const string& name,
             ...
             pthread_mutex_t* mutexapp,
             bool flag) {
  unsigned long int point_count, max_avail_memory, max_texture_dimension,
      needed_mem;
...
////////////////////////////////////////////////////////////////////////////////
//
void QTC(const string& name,
         int matrix_type,
         cudaStream_t stream_app,
         pthread_mutex_t* mutexapp,
         bool flag) {
  ofstream debug_out, seeds_out;
  void *Ai_mask, *cardnl, *ungrpd_pnts_indr, *clustered_pnts_mask, *result,
      *dist_to_clust;
  ...
  float threshold;
  int i, max_degree, thread_block_count, total_thread_block_count,
      active_node_count;
  ...
  if (cwrank == 0) {
    // if( synthetic_data )
    pnts = generate_synthetic_data(&dist_source, &indr_mtrx_host, &max_degree,
                                   threshold, point_count, matrix_type);
    // else
    //(void)read_BLAST_data(&dist_source, &indr_mtrx_host, &max_degree,
    // threshold, fname.c_str(), point_count, matrix_type);
  }
  ...
  if (cwrank != 0) {  // For all nodes except zero, in a distributed run.
    ...
    allocHostBuffer((void**)&indr_mtrx_host,
                    point_count * max_degree * sizeof(int));
  }
  // If we need to print the actual clusters later on, we'll need to have all
  // points in all nodes.
  if (save_clusters) {
    if (cwrank != 0) {
  ...
  calculate_participants(point_count, node_count, cwrank, &thread_block_count,
                         &total_thread_block_count, &active_node_count);
  ...
  if (can_use_texture) {
    ...
    // This is the actual distance matrix (dst_matrix_elems should be
    // "point_count^2, or point_count*max_degree)
    ...
    cudaMallocArray(&distance_matrix_txt, &texDistance.channelDesc,
                    dst_matrix_elems / point_count, point_count);
  } else {
  ...
  allocDeviceBuffer(&dist_to_clust,
                    thread_block_count * max_degree * sizeof(float));
  ...
  if (can_use_texture) {
    cudaMemcpyToArrayAsync(distance_matrix_txt, 0, 0, dist_source,
                      dst_matrix_elems * sizeof(float), cudaMemcpyHostToDevice, stream_app);
    CHECK_CUDA_ERROR();
    cudaBindTextureToArray(texDistance, distance_matrix_txt);
  } else {
    copyToDevice(distance_matrix_gmem, dist_source,
                 dst_matrix_elems * sizeof(float), stream_app);
  }

  copyToDevice(indr_mtrx, indr_mtrx_host,
               point_count * max_degree * sizeof(int), stream_app);

  copyToDevice(ungrpd_pnts_indr, ungrpd_pnts_indr_host,
               point_count * sizeof(int), stream_app);
  ...
  compute_degrees<<<grid2D(thread_block_count), tpb, 0, stream_app>>>(
      (int*)indr_mtrx, (int*)degrees, point_count, max_degree);
  ...
  do {
    ...
    // If there are only a few elements left to cluster, reduce the number of
    // participating nodes (GPUs).
    ...
    ///////// -----------------               Main kernel
    ///----------------- /////////
    QTC_device<<<grid, tpb, 0, stream_app>>>(
        (float*)distance_matrix, (char*)Ai_mask, (char*)clustered_pnts_mask,
    ...
    if (thread_block_count > 1) {
      pthread_mutex_lock(mutexapp);
      // We are reducing 128 numbers or less, so one thread should be
      // sufficient.
      reduce_card_device<<<grid2D(1), 1, 0, stream_app>>>((int*)cardnl,
                                                          thread_block_count);
      pthread_mutex_unlock(mutexapp);
      if (flag)
        cutilSafeCall(cudaStreamSynchronize(stream_app));
    ...
    // comm_find_winner(&max_card, &winner_node, &winner_index, cwrank,
    // max_point_count+1);
    // t_comm += Timer::Stop(Tcomm, "Comm Only");

    if (be_verbose && cwrank == winner_node) {  // for non-parallel cases, both
                                                // "cwrank" and "winner_node"
                                                // should be zero.
      cout << "[" << cwrank << "] Cluster Cardinality: " << max_card
           << " (Node: " << cwrank << ", index: " << winner_index << ")"
           << endl;
    }

    // int Ttrim = Timer::Start();
    pthread_mutex_lock(mutexapp);
    trim_ungrouped_pnts_indr_array<<<grid2D(1), tpb, 0, stream_app>>>(
        ...
        point_count, max_point_count, max_degree, threshold, matrix_type,
        can_use_texture);
    ...
    if (cwrank ==
        winner_node) {  // for non-parallel cases, these should both be zero.
      if (save_clusters) {
        ss << "p." << iter;
      ...
      if (save_clusters) {
        for (int i = 0; i < max_card; i++) {
          debug_out << pnts[2 * output[i]] << " " << pnts[2 * output[i] + 1]
                    << endl;
        }
        seeds_out << pnts[2 * winner_index] << " " << pnts[2 * winner_index + 1]
                  << endl;
        debug_out.close();
      }
    }
    ...
    pthread_mutex_lock(mutexapp);
    update_clustered_pnts_mask<<<grid2D(1), tpb, 0, stream_app>>>(
        (char*)clustered_pnts_mask, (char*)Ai_mask, max_point_count);
    pthread_mutex_unlock(mutexapp);
    if (flag)
      cutilSafeCall(cudaStreamSynchronize(stream_app));
    else
...
void copyToDevice(void* to_device,
                  ...
                  unsigned long bytes,
                  cudaStream_t stream_app) {
  cudaMemcpyAsync(to_device, from_host, bytes, cudaMemcpyHostToDevice,
                  stream_app);
  CHECK_CUDA_ERROR();
}
...
void copyFromDevice(void* to_host,
                    ...
                    unsigned long bytes,
                    cudaStream_t stream_app) {
  cudaMemcpyAsync(to_host, from_device, bytes, cudaMemcpyDeviceToHost,
                  stream_app);
  CHECK_CUDA_ERROR();
}
