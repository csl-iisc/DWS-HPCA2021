...
* code.
*  This is done because any conventional passing through argument list way
* results
...
float encode(byte* ImgSrc,
  ...
  float* SrcDst;
  size_t DeviceStride;
  cutilSafeCall(cudaMallocPitch((void**)(&SrcDst), &DeviceStride,
                                Size.width * sizeof(float), Size.height));
  DeviceStride /= sizeof(float);

  // copy from host memory to device
  cutilSafeCall(cudaMemcpy2D(
      SrcDst, DeviceStride * sizeof(float), ImgF1, StrideF * sizeof(float),
  ...
  CUDAkernel2DCT<<<GridFullWarps, ThreadsFullWarps, 0, stream_app>>>(
  ...
  CUDAkernelQuantizationFloat<<<GridSmallBlocks, ThreadsSmallBlocks, 0,
                                stream_app>>>(SrcDst, (int)DeviceStride);
  ...
  cutilSafeCall(cudaMemcpy2D(
      ImgF1, StrideF * sizeof(float), SrcDst, DeviceStride * sizeof(float),
...
float decode(byte* ImgSrc,
  ...
  // copy from host memory to device
  cutilSafeCall(cudaMemcpy2D(
      SrcDst, DeviceStride * sizeof(float), ImgF2, StrideF * sizeof(float),
      Size.width * sizeof(float), Size.height, cudaMemcpyHostToDevice));
  ...
  cutilCheckError(cutStartTimer(timerCUDA));
  CUDAkernel2IDCT<<<GridFullWarps, ThreadsFullWarps, 0, stream_app>>>(
      SrcDst, (int)DeviceStride);
  ...
  // copy quantized image block to host
  cutilSafeCall(cudaMemcpy2D(
      ImgF2, StrideF * sizeof(float), SrcDst, DeviceStride * sizeof(float),
      Size.width * sizeof(float), Size.height, cudaMemcpyDeviceToHost));
  cutilCheckError(cutStopTimer(timerCUDA));
  float TimerCUDASpan = cutGetAverageTimerValue(timerCUDA);
  cutilCheckError(cutDeleteTimer(timerCUDA));
...
// int main(int argc, char** argv)
{
  ...
  /*if(cutCheckCmdLineFlag(argc, (const char**)argv, "encode")) {
   ...
   if(!cutGetCmdLineArgumentstr(argc, (const char**)argv, "file",
  &SampleImageFname)) {
     usage();
   }
  ...
  ROI ImgSize;
  int res = PreLoadBmp("/home/pratheek-htc/masksim/v3.x/pthread_benchmark/JPEG/data/image.bmp", &ImgWidth, &ImgHeight);
  ...
  // load sample image
  LoadBmpAsGray("/home/pratheek-htc/masksim/v3.x/pthread_benchmark/JPEG/data/image.bmp", ImgStride, ImgSize, ImgSrc);
  ...
  float time;
  if (encoding) {
    time = encode(ImgSrc, ImgDstCUDA1, ImgStride, ImgSize, stream_app, mutexapp,
                  flag);
  } else {
    time = decode(ImgSrc, ImgDstCUDA1, ImgStride, ImgSize, stream_app, mutexapp,
                  flag);
  }
  printf("%f\n", time);
