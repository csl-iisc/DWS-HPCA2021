...
void RunTest(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  CUDA_SAFE_CALL(cudaMemcpyAsync(d_idata, h_idata, bytes,
  ...
  for (int k = 0; k < passes; k++) {
    ...
    for (int j = 0; j < iters; j++) {
      ...
      bottom_scan<T,
                  vecT><<<num_blocks, num_threads, 2 * smem_size, stream_app>>>(
    ...
    // resultDB.AddResult(testName+"_PCIe", atts, "GB/s",gb / (avgTime +
    // transferTime));
