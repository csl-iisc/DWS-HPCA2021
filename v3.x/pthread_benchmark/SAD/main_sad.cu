...
int main_sad(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  /* Run the kernel code */
  {
    ...
    CUDA_ERRCK
    cudaMallocArray(&ref_ary, &get_ref().channelDesc, ref_image->width,
                    ref_image->height);
    CUDA_ERRCK
    ...
    cudaMemcpy2DToArrayAsync(ref_ary, 0, 0, ref_image->data,
                        ref_image->width * sizeof(unsigned short),
                        ref_image->width * sizeof(unsigned short),
                        ref_image->height, cudaMemcpyHostToDevice, stream_app);
    CUDA_ERRCK
    ...
    /* mb_sad_calc<<<dim3(CEIL(ref_image->width / 4, THREADS_W), */
        ...
        /* d_sads, (unsigned short*)d_cur_image, image_width_macroblocks, */
        /* image_height_macroblocks); */
    /* CUDA_ERRCK */

    /* Run the larger-blocks kernels */
    larger_sad_calc_8<<<macroblock_grid, dim3(32, 4), 0, stream_app>>>(
        d_sads, image_width_macroblocks, image_height_macroblocks);
    CUDA_ERRCK
