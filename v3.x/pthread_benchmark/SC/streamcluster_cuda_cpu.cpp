  ...
  while (change / cost > 1.0 * e) {
    ...
    for (i = 0; i < iter; i++) {
      ...
      change += pgain(feasible[x], points, z, k, kmax, is_center, center_table,
                      ...
                      &gpu_malloc, &kernel, stream_app, mutexapp, flag);
      // pthread_mutex_lock (mutexapp); // not sure
    }
...
int selectfeasible_fast(Points* points,
                        int** feasible,
                        int kmin,
                        int pid,
                        pthread_barrier_t* barrier) {
...
void copycenters(Points* points,
                 Points* centers,
                 long* centerIDs,
                 long offset) {
  long i;
...
void* localSearchSub(void* arg_) {
  ...
  pkmedian(arg->points, arg->kmin, arg->kmax, arg->kfinal, arg->pid,
           arg->barrier, arg->stream_app, arg->mutexapp, arg->flag);

  return NULL;
}

void localSearch(Points* points,
                 long kmin,
                 long kmax,
                 long* kfinal,
                 cudaStream_t stream_app,
  ...
  for (int i = 0; i < nproc; i++) {
    ...
    arg[i].stream_app = stream_app;
    arg[i].mutexapp = mutexapp;
    arg[i].flag = flag;
...
void streamCluster(PStream* stream,
  ...
  while (1) {
    ...
    localSearch(&points, kmin, kmax, &kfinal, stream_app, mutexapp, flag);
  ...
  localSearch(&centers, kmin, kmax, &kfinal, stream_app, mutexapp, flag);
  contcenters(&centers);
  outcenterIDs(&centers, centerIDs, outfile);
}

// int main(int argc, char **argv)
int main_streamcluster(cudaStream_t stream_app,
                       pthread_mutex_t* mutexapp,
                       bool flag) {
