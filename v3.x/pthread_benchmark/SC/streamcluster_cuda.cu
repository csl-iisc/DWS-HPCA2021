...
/* kernel */
__global__ void pgain_kernel(int num,
                             int dim,
                             long x,
                             Point* p,
                             int K,
  ...
  extern __shared__ float
      coord_s[];  // shared memory for coordinate of point[x]
...
void allocDevMem(int num, int dim, int kmax) {
  if (cudaMalloc((void**)&work_mem_d, kmax * num * sizeof(float)) !=
      cudaSuccess)
    quit("error allocating device memory");
  if (cudaMalloc((void**)&center_table_d, num * sizeof(int)) != cudaSuccess)
    quit("error allocating device memory");
  if (cudaMalloc((void**)&switch_membership_d, num * sizeof(bool)) !=
      cudaSuccess)
    quit("error allocating device memory");
  if (cudaMalloc((void**)&p, num * sizeof(Point)) != cudaSuccess)
    quit("error allocating device memory");
  if (cudaMalloc((void**)&coord_d, num * dim * sizeof(float)) != cudaSuccess)
    quit("error allocating device memory");
}
...
float pgain(long x,
  ...
  *serial += t2 - t1;
  ...
  if (c == 0) {
    ...
    *serial += t5 - t4;
    ...
    cudaMemcpyAsync(switch_membership_d, switch_membership, num * sizeof(bool),
                    cudaMemcpyHostToDevice, stream_app);
    cudaMemcpyAsync(coord_d, coord_h, num * dim * sizeof(float),
                    cudaMemcpyHostToDevice, stream_app);
#ifdef PROFILE
    double t6 = gettime();
  ...
  cudaMemcpyAsync(center_table_d, center_table, num * sizeof(int),
                  cudaMemcpyHostToDevice, stream_app);
  cudaMemcpyAsync(p, points->p, num * sizeof(Point), cudaMemcpyHostToDevice,
                  stream_app);
  ...
  *kernel += t10 - t9;
  ...
  cudaMemcpyAsync(work_mem_h, work_mem_d, (K + 1) * num * sizeof(float),
                  cudaMemcpyDeviceToHost, stream_app);
  cudaMemcpyAsync(switch_membership, switch_membership_d, num * sizeof(bool),
                  cudaMemcpyDeviceToHost, stream_app);
