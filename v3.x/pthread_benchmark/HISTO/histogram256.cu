...
inline __device__ void addByte(volatile uint* s_WarpHist,
                               uint data,
                               uint threadTag) {
  uint count;
  do {
...
__global__ void histogram256Kernel(uint* d_PartialHistograms,
                                   uint* d_Data,
                                   uint dataCount) {
...
extern "C" void histogram256(cudaStream_t stream_app,
  ...
  histogram256Kernel<<<PARTIAL_HISTOGRAM256_COUNT,
                       HISTOGRAM256_THREADBLOCK_SIZE, 0, stream_app>>>(
  ...
  mergeHistogram256Kernel<<<HISTOGRAM256_BIN_COUNT, MERGE_THREADBLOCK_SIZE, 0,
                            stream_app>>>(d_Histogram, d_PartialHistograms,
...
extern "C" void initBuffer_HIST(cudaStream_t stream_app) {
  ...
  cutilSafeCall(cudaMemcpyAsync(d_Data, h_Data, byteCount,
...
int main_histo(cudaStream_t stream_app, pthread_mutex_t* mutexapp, bool flag) {
  ...
  cutilSafeCall(cudaMemcpyAsync(h_HistogramGPU, d_Histogram,
