...
SimulationData move_simulation_data_to_device( Inputs in, int mype, SimulationData SD , cudaStream_t stream_app )
{
	...
	gpuErrchk( cudaMemcpyAsync(GSD.num_nucs, SD.num_nucs, sz, cudaMemcpyHostToDevice, stream_app) );
	total_sz += sz;

	sz = GSD.length_concs * sizeof(double);
	gpuErrchk( cudaMalloc((void **) &GSD.concs, sz) );
	gpuErrchk( cudaMemcpyAsync(GSD.concs, SD.concs, sz, cudaMemcpyHostToDevice, stream_app) );
	total_sz += sz;

	sz = GSD.length_mats * sizeof(int);
	gpuErrchk( cudaMalloc((void **) &GSD.mats, sz) );
	gpuErrchk( cudaMemcpyAsync(GSD.mats, SD.mats, sz, cudaMemcpyHostToDevice, stream_app) );
	total_sz += sz;
	
	sz = GSD.length_unionized_energy_array * sizeof(double);
	gpuErrchk( cudaMalloc((void **) &GSD.unionized_energy_array, sz) );
	gpuErrchk( cudaMemcpyAsync(GSD.unionized_energy_array, SD.unionized_energy_array, sz, cudaMemcpyHostToDevice, stream_app) );
	total_sz += sz;

	sz = GSD.length_index_grid * sizeof(int);
	gpuErrchk( cudaMalloc((void **) &GSD.index_grid, sz) );
	gpuErrchk( cudaMemcpyAsync(GSD.index_grid, SD.index_grid, sz, cudaMemcpyHostToDevice, stream_app) );
	total_sz += sz;

	sz = GSD.length_nuclide_grid * sizeof(NuclideGridPoint);
	gpuErrchk( cudaMalloc((void **) &GSD.nuclide_grid, sz) );
	gpuErrchk( cudaMemcpyAsync(GSD.nuclide_grid, SD.nuclide_grid, sz, cudaMemcpyHostToDevice, stream_app) );
	total_sz += sz;
