...
int main_XSBENCH(cudaStream_t stream_app,
  ...
  char** argv;
	...
	// Move data to GPU
	SimulationData GSD = move_simulation_data_to_device( in, mype, SD , stream_app);
	...
	{
		if( in.kernel_id == 0 )
			verification = run_event_based_simulation_baseline(in, GSD, mype, stream_app);
