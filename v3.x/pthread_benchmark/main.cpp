...
struct app_data
{
  app_data(char* app_name, pthread_mutex_t* app_mutex, bool concurrent,
      ...
      app_name(app_name), app_mutex(app_mutex), concurrent(concurrent),
...
int callApp(char *app_name, cudaStream_t stream_app, pthread_mutex_t *mutexapp,
    bool concurrent)
{
  if (strcmp(app_name, "NN") == 0)
    main_NN(stream_app, mutexapp, concurrent);
  else if (strcmp(app_name, "ATAX") == 0)
    main_ATAX(stream_app, mutexapp, concurrent);
  else if (strcmp(app_name, "XSBENCH") == 0)
    main_XSBENCH(stream_app, mutexapp, concurrent);
  ...
  else if (strcmp(app_name, "MICRO") == 0)
    main_micro(stream_app, mutexapp, concurrent);
  else
    return EXIT_FAILURE;
  return EXIT_SUCCESS;
}
...
void* benchmark(void *app_arg)
{
  ...
  do
  {
    ...
    printf("Launch code in main.cu:launching a new benchmark, appID = %d,\
already registered? = %d\n", app->appID, App::is_registered(app->appID));
    if(App::is_registered(app->appID)) 
    {
      thread_to_appID->add((void*)pthread_self(), App::get_app_id(app->appID));
      App::set_app_name(app->appID, name);
      printf("AppID_to_AppName: %d -> %s\n", App::get_app_id(app->appID),
          app->app_name);
    }
    else 
    {
      thread_to_appID->add((void*)pthread_self(), App::register_app(app->appID));
      App::set_app_name(app->appID, name);
      printf("AppID_to_AppName: %d -> %s\n", App::get_app_id(app->appID),
          app->app_name);
    }
...
void* benchmark_one(void *app_arg)
{
  struct app_data* app = (struct app_data*) app_arg;
  char* name = app->app_name;
  int ret;
...
int main(int argc, char *argv[])
{
  ...
  thread_to_appID = (thread_to_appID_struct *)
    malloc(sizeof(thread_to_appID_struct));
  ...
  for (size_t i = 0; i < n_apps; i++)
  {
    ...
    apps.push_back(app_data(argv[i + 1], &app_mutex, concurrent,
          &done_events[i], &done_events, i));
  }
  for (size_t i = 0; i < n_apps; i++)
  {
    if(n_apps == 1)
      errno = pthread_create(&threads[i], NULL, benchmark_one, &apps[i]);
    else
